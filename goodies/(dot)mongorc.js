...
// not cryptographic, very collision prone
function hash(z) {return ("00000000" + Buffer.from(EJSON.stringify(z) + "----").reduce(function(a,x){return ((a << 5) - a + x) >>> 0},0).toString(16)).substr(-8)}
...
// return an array with every possible case combination of the input string
var reCase = function (s, a=[""]) {
    ...
    l = c.toLowerCase();
    newa = a.map(e=>e+u);
    if (u != l) {newa = newa.concat(a.map(e => e+l ))};
    return reCase(r,newa)
}
...
var mod10_check = function(number) {
    try {
        ...
        return (string.length > 1) && 
            // negative numbers are not welcome here
               (parseInt(string) >= 0) && 
               string.split("").map(c => parseInt(c))
                 .reverse()
                 .reduce( function (sum,digit,idx) {
                     // alternate adding the digit, or the sum of the digits of its double
                            let d = digit;
                            if (idx % 2 != 0) {
                               d = d * 2;
                            }
                            ...
                            return (sum + d); 
                 }) % 10 == 0 // the overall sum must be a multiple of 10
    } catch (e) {
        // if something doesn't work, meh
        return false
    }
}
...
async function connectionCSV(filename, dbname, collname) {
    ...
    while (cursor.hasNext()) {
        ...
        var linedata = [
            ...
            ((obj.receivedFirst.length>0)?obj.receivedFirst.filter(e=>e).map(d => d.toISOString())[0]:"") || "",
